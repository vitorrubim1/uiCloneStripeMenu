...
export function DropDownRoot() {
  const { options, cacheId, getOptionById, targetId } = React.useContext(
    Context
  );

  const cachedOption = React.useMemo(() => getOptionById(cacheId), [
    cacheId,
    getOptionById,
  ]);
  ...
  const isActive = targetId !== null || hovering; //container ativo
...
function DropDownArrow({ isFirstInteraction }) {
  const { cacheId, getOptionById } = React.useContext(Context);
